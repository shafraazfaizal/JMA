...
import { ArrowLeft, Clock, Calendar, User, ArrowRight, Share2, Image as ImageIcon } from "lucide-react";
...
// Maps a news category to the most relevant gallery filter
        ...
        <main style={{ minHeight: "100vh", backgroundColor: "#ffffff" }}>
            ...
            <section style={{ background: "linear-gradient(160deg, #0D5C6B 0%, #1a7a8f 40%, #073D47 100%)", padding: "8rem 1.5rem 4rem", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ maxWidth: "760px", margin: "0 auto", position: "relative", zIndex: 1 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "1.5rem", flexWrap: "wrap" as const }}>
                        ...
                        <span style={{ display: "flex", alignItems: "center", gap: "0.4rem", fontFamily: "var(--font-inter)", fontSize: "0.875rem", color: "rgba(255,255,255,0.65)" }}>
                            <Calendar size={14} aria-hidden="true" /> {fmtDate(article.publishedAt)}
                        </span>
                        <span style={{ display: "flex", alignItems: "center", gap: "0.4rem", fontFamily: "var(--font-inter)", fontSize: "0.875rem", color: "rgba(255,255,255,0.65)" }}>
                            <Clock size={14} aria-hidden="true" /> {article.readTime} min read
                        </span>
                    </div>
                </div>
            </section>
            ...
            <section style={{ padding: "4rem 1.5rem" }}>
                <div style={{ maxWidth: "760px", margin: "0 auto" }}>
                    ...
                    {/* Image placeholder */}
                    <div style={{ height: "320px", borderRadius: "1rem", background: "linear-gradient(135deg, #0D5C6B 0%, #1a7a8f 50%, #073D47 100%)", marginBottom: "2.5rem", position: "relative", overflow: "hidden" }}>
                    ...
                    <div style={{ marginTop: "3rem", paddingTop: "2rem", borderTop: "1px solid #E5E7EB", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" as const, gap: "1rem" }}>
                        <p style={{ fontFamily: "var(--font-inter)", fontSize: "0.875rem", color: "#9CA3AF" }}>
                            Published {fmtDate(article.publishedAt)} by {article.author}
                        </p>
                        <button
                            onClick={() => {
                                ...
                                return (
                                    <Link key={a.id} href={`/news/${a.slug}`} style={{ textDecoration: "none" }}>
                                        ...
                                        >
                                            ...
                                            <p style={{ fontFamily: "var(--font-inter)", fontSize: "0.75rem", color: "#9CA3AF" }}>{fmtDate(a.publishedAt)}</p>
                                        </article>
                                    </Link>
                                );
                            })}
