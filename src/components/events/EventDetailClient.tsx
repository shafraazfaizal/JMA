...
import {
    ...
    Share2, CheckCircle, Mail, User, AlertCircle,
} from "lucide-react";
import { submitRSVPAction } from "@/app/admin/events/actions";
...
export default function EventDetailClient({ event, isPast, related }: EventDetailClientProps) {
            ...
            <section style={{ background: "linear-gradient(160deg, #0D5C6B 0%, #1a7a8f 40%, #073D47 100%)", padding: "8rem 1.5rem 4rem", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ maxWidth: "760px", margin: "0 auto", position: "relative", zIndex: 1 }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap" as const, gap: "1.5rem" }}>
                        {[
                            { icon: Calendar, value: fmtDate(event.event_date) },
                            { icon: Clock, value: event.event_time },
                            ...
                            {isPast ? (
                                ...
                                <div style={{ textAlign: "center" as const, padding: "0.5rem 0" }}>
                                    ...
                                    <p style={{ fontFamily: "var(--font-inter)", fontSize: "0.8125rem", color: "#6B7280", lineHeight: 1.6 }}>
                                        We&apos;ve recorded your RSVP for <strong style={{ color: "#111827" }}>{rsvpEmail}</strong>. We look forward to seeing you there.
                                    </p>
                                </div>
                            ) : (
                                <>
                                    ...
                                    {error && (
                                        <div style={{ display: "flex", alignItems: "center", gap: "0.375rem", marginBottom: "0.875rem" }}>
                                            <AlertCircle size={13} style={{ color: "#EF4444", flexShrink: 0 }} aria-hidden="true" />
                                            <p style={{ fontFamily: "var(--font-inter)", fontSize: "0.8125rem", color: "#EF4444" }}>{error}</p>
                                        </div>
                                    )}
                ...
                <section style={{ padding: "4rem 1.5rem", backgroundColor: "#F9FAFB" }}>
                    <div style={{ maxWidth: "80rem", margin: "0 auto" }}>
                        <h2 style={{ fontFamily: "var(--font-jakarta)", fontWeight: 700, fontSize: "1.25rem", color: "#111827", marginBottom: "1.75rem" }}>
                            {isPast ? "More past events" : "More upcoming events"}
                        </h2>
                            ...
                            {related.map((e) => (
                                <Link key={e.id} href={`/events/${e.slug}`} style={{ textDecoration: "none" }}>
                                    ...
                                    >
                                        ...
                                        <p style={{ fontFamily: "var(--font-inter)", fontSize: "0.8125rem", color: "#9CA3AF" }}>{fmtDate(e.event_date)} · {e.location}</p>
                                    </article>
                                </Link>
                            ))}
                        </div>
