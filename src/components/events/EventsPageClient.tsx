...
export default function EventsPageClient({ events }: { events: DBEvent[] }) {
    const [tab, setTab] = useState<"upcoming" | "past">("upcoming");

    const { upcoming, past } = useMemo(() => {
        const upcoming = events
            .filter((e) => !isEventPast(e.event_date))
                ...
                <div style={{ maxWidth: "80rem", margin: "0 auto" }}>
                    {list.length === 0 ? (
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "1.5rem" }} className="events-grid">
                            {list.map((event) => {
                                const { day, month } = fmtDateShort(event.event_date);
                                    ...
                                    <Link key={event.id} href={`/events/${event.slug}`} style={{ textDecoration: "none" }}>
                                        <article
                                            ...
                                            onMouseEnter={(e) => {
                                                ...
                                                if (!isPast) el.style.transform = "translateY(-2px)";
                                            }}
                                            onMouseLeave={(e) => {
                                                const el = e.currentTarget as HTMLElement;
                                        ...
                                        >
                                            <div style={{ flexShrink: 0, width: "64px", height: "64px", borderRadius: "0.875rem", backgroundColor: isPast ? "#F3F4F6" : "#E8F4F6", display: "flex", flexDirection: "column" as const, alignItems: "center", justifyContent: "center" }}>
                                                <span style={{ fontFamily: "var(--font-jakarta)", fontWeight: 800, fontSize: "1.25rem", color: isPast ? "#9CA3AF" : "#0D5C6B", lineHeight: 1 }}>{day}</span>
                                                <span style={{ fontFamily: "var(--font-inter)", fontWeight: 600, fontSize: "0.6875rem", color: isPast ? "#9CA3AF" : "#0D5C6B", letterSpacing: "0.05em" }}>{month}</span>
                                            </div>
                                            ...
                                            <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" as const, gap: "0.625rem" }}>
                                                ...
                                                <div style={{ display: "flex", flexWrap: "wrap" as const, gap: "0.25rem 1rem", fontFamily: "var(--font-inter)", fontSize: "0.8125rem", color: "#9CA3AF" }}>
                                                    <span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}><Clock size={12} aria-hidden="true" /> {event.event_time}</span>
